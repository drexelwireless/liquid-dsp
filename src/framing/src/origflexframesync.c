...
//  _x      :   input sample
//  _y      :   output symbol
int origflexframesync_update_symsync(origflexframesync   _q,
                                     float complex   _x,
                                     float complex * _y);

// push buffered p/n sequence through synchronizer
void origflexframesync_pushpn(origflexframesync _q);

// push samples through synchronizer, saving received p/n symbols
void origflexframesync_execute_rxpn(origflexframesync _q,
                                    float complex _x);

// once p/n symbols are buffered, estimate residual carrier
// frequency and phase offsets, push through fine-tuned NCO
void origflexframesync_syncpn(origflexframesync _q);

// receive header symbols
void origflexframesync_execute_rxheader(origflexframesync _q,
                                        float complex _x);

// receive payload symbols
void origflexframesync_execute_rxpayload(origflexframesync _q,
                                         float complex _x);
...
origflexframesync origflexframesync_create(framesync_callback _callback,
                                           void *            _userdata)
{
    origflexframesync q = (origflexframesync) malloc(sizeof(struct origflexframesync_s));
    ...
    firinterp_crcf interp = firinterp_crcf_create_prototype(LIQUID_FIRFILT_ARKAISER,q->k,q->m,q->beta,0);
    for (i=0; i<64+q->m; i++) {
                               ...
                               unsigned int    _n)
{
    ...
    for (i=0; i<_n; i++) {
        ...
        case STATE_DETECTFRAME:
            ...
            origflexframesync_execute_seekpn(_q, _x[i]);
            break;
        case STATE_RXPN:
            // receive p/n sequence symbols
            origflexframesync_execute_rxpn(_q, _x[i]);
            break;
        case STATE_RXHEADER:
            // receive header sequence symbols
            origflexframesync_execute_rxheader(_q, _x[i]);
            break;
        case STATE_RXPAYLOAD:
            // receive payload symbols
            origflexframesync_execute_rxpayload(_q, _x[i]);
            break;
        default:
            fprintf(stderr,"error: origflexframesync_exeucte(), unknown/unsupported state\n");
            exit(1);
        }
    }
}
...
void origflexframesync_pushpn(origflexframesync _q)
{
    ...
    for (i=0; i<buffer_len; i++) {
        if (i < delay) {
            ...
            origflexframesync_execute_rxpn(_q, rc[i]);
        }
    }
                                    ...
                                    float complex _x)
{
    // validate input
    if (_q->pn_counter == 64) {
        fprintf(stderr,"warning: origflexframesync_execute_rxpn(), p/n buffer already full!\n");
        return;
    }
    ...
    int sample_available = origflexframesync_update_symsync(_q, y, &mf_out);
...
void origflexframesync_decode_header(origflexframesync _q)
{
    ...
    if (_q->header[n+0] != ORIGFLEXFRAME_VERSION) {
        fprintf(stderr,"warning: origflexframesync_decode_header(), invalid framing version\n");
        _q->header_valid = 0;
        return;
    }
    ...
    if (mod_scheme == 0 || mod_scheme >= LIQUID_MODEM_NUM_SCHEMES) {
        fprintf(stderr,"warning: origflexframesync_decode_header(), invalid modulation scheme\n");
        _q->header_valid = 0;
        return;
    }
    if (check >= LIQUID_CRC_NUM_SCHEMES) {
        fprintf(stderr,"warning: origflexframesync_decode_header(), decoded CRC exceeds available\n");
        _q->header_valid = 0;
        return;
    }
    if (fec0 >= LIQUID_FEC_NUM_SCHEMES) {
        fprintf(stderr,"warning: origflexframesync_decode_header(), decoded FEC (inner) exceeds available\n");
        _q->header_valid = 0;
        return;
    }
    if (fec1 >= LIQUID_FEC_NUM_SCHEMES) {
        fprintf(stderr,"warning: origflexframesync_decode_header(), decoded FEC (outer) exceeds available\n");
        _q->header_valid = 0;
        return;
    }
    ...
    if (_q->header_valid) {
        ...
        if (_q->payload_mod == NULL || _q->payload_enc == NULL || _q->payload_dec == NULL) {
            fprintf(stderr,"error: origflexframesync_decode_header(), could not re-allocate payload arrays\n");
            _q->header_valid = 0;
            return;
        }
    }
...
#if DEBUG_ORIGFLEXFRAMESYNC_PRINT
    // print results
    printf("origflexframesync_decode_header():\n");
...
#else
    fprintf(stderr,"origflexframesync_debug_enable(): compile-time debugging disabled\n");
#endif
}
...
void origflexframesync_debug_printorig(origflexframesync  _q,
                                       const char * _filename)
{
#if DEBUG_FLEXFRAMESYNC
    if (!_q->debug_objects_created) {
        fprintf(stderr,"error: origflexframesync_debug_print(), debugging objects don't exist; enable debugging first\n");
        return;
    }
    unsigned int i;
    float complex * rc;
...
#else
    fprintf(stderr,"origflexframesync_debug_print(): compile-time debugging disabled\n");
#endif
}
